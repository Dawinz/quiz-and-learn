...
class EnhancedQuizScreen extends StatefulWidget {
  ...
  final int timeLimit; // Total quiz time in seconds
  final int questionTimeLimit; // Individual question time in seconds

  const EnhancedQuizScreen({
    super.key,
    required this.category,
    required this.difficulty,
    this.timeLimit = 0, // 0 for no limit
    this.questionTimeLimit = 0, // 0 for no limit
  });
...
class _EnhancedQuizScreenState extends State<EnhancedQuizScreen> {
  final EnhancedQuizService _quizService = EnhancedQuizService();
  ...
  void _togglePause() {
    if (_isPaused) {
      _quizService.resumeQuiz();
      setState(() {
        _isPaused = false;
      });
    } else {
      _quizService.pauseQuiz();
      setState(() {
        _isPaused = true;
      });
    }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            tooltip: _isPaused ? 'Resume Quiz' : 'Pause Quiz',
          ),
          // Question counter
          Container(
                      ...
                      Column(
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              _formatTime(_quizService.quizTimeRemaining),
                              style: TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.bold,
                                color: _quizService.quizTimeRemaining < 30
                      ...
                      Column(
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              _formatTime(_quizService.questionTimeRemaining),
                              style: TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.bold,
                                color: _quizService.questionTimeRemaining < 10
