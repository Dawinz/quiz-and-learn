...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      ...
      if (!hasInternet) {
        ConnectivityService.showNetworkErrorSnackBar(context,
            'No internet connection. Please check your connection and try again.');
        return;
      }

      // Load earnings summary and pending withdrawals
      await Future.wait([
    ...
    try {
      ...
      if (response['success']) {
        ...
        _earningsSummary = {
          ...
          'pendingWithdrawals': data['pendingWithdrawals'] ?? 0.0,
        };
      } else {
  ...
  Future<void> _loadPendingWithdrawals() async {
    try {
      final response = await _apiService.getWithdrawals();
      if (response['success']) {
        final withdrawals = response['data'] ?? [];
        _pendingWithdrawals =
            withdrawals.where((w) => w['status'] == 'pending').take(3).toList();
      } else {
        _pendingWithdrawals = [];
      }
    } catch (e) {
